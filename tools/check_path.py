    ...
    for method_name, method_info in methods_info.items():
        ...
        for dataset_name, resutls_info in method_info.items():
            ...
            if not os.path.exists(dir_path):
                ...
                continue
            elif len(pred_names := [name[:-4] for name in os.listdir(dir_path) if name.endswith(file_suffix)]) == 0:
                total_msgs.append(f"{method_name}/{dataset_name}: {dir_path} 中不包含后缀为{file_suffix}的文件")
                continue

            mask_names = [name[:-4] for name in os.listdir(mask_path) if name.endswith(mask_suffix)]
            if len(set(mask_names).intersection(set(pred_names))) == 0:
                total_msgs.append(f"{method_name}/{dataset_name}: {dir_path} 中数据名字与真值 {mask_path} 不匹配")
